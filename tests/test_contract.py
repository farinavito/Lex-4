...
def test_sellerProducts_after_initialization_2(deploy, not_seller):
    ...
    try:
        for num in range(1, 5):
            assert deploy.sellerProducts(accounts[not_seller], num) == 0
...
@pytest.mark.parametrize("buyer_or_seller", [buyer, seller])
def test_withdrawAsTheBuyer_first_reqirements_2(deploy, buyer_or_seller):
    '''test if the first requirement works'''
    deploy.payOut(1, {'from': accounts[buyer_or_seller]})
...
@pytest.mark.parametrize("buyer_or_seller", [buyer, seller])
def test_withdrawAsTheBuyer_send_amount(deploy, buyer_or_seller):
...
@pytest.mark.parametrize("buyer_or_seller", [buyer, seller])
def test_withdrawAsTheBuyer_amount_to_0(deploy, buyer_or_seller):
...
@pytest.mark.parametrize("buyer_or_seller", [buyer, seller])
def test_withdrawAsTheBuyer_emit_event(deploy, buyer_or_seller):
